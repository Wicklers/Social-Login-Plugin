...
class FacebookCurlHttpClient implements FacebookHttpable {
    ...
    public function openConnection($url, $method = 'GET', $parameters = array()) {
        if (PROXY_SETTINGS=="1") {
            $options = array(
                            ...
                            CURLOPT_HTTPPROXYTUNNEL => 1, //IF USING PROXY TO CONNECT
                            CURLOPT_PROXY => PROXY_RULE //SET PROXY
                            );
                          //  die("here");
        }
        else{
    ...
    public function addBundledCert() {
        self::$facebookCurl -> setopt(CURLOPT_CAINFO, dirname(__FILE__) . DIRECTORY_SEPARATOR . 'fb_ca_chain_bundle.crt');
    }
