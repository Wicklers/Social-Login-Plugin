...
class Install_Model {
    ...
    public function step2(){
        $this->create_config_file(Session::get('db-server'), Session::get('db-username'), Session::get('db-password'), Session::get('db-name'), Session::get('db-table'), Input::get('fb-app-id'), Input::get('fb-app-secret-id'));
        Session::destroy();
        return 1;
    }

    public function create_config_file($db_server, $db_username, $db_password, $db_name, $db_table, $fb_app_id, $fb_app_secret_id) {
        ...
        while (!feof($file)) {
            $line = fgets($file);
            $var = array("db-server", "db-username", "db-password", "db-name", "db-table", "fb-app-id","fb-app-secret-id");
            $values = array($db_server, $db_username, $db_password, $db_name, $db_table, $fb_app_id, $fb_app_secret_id);
